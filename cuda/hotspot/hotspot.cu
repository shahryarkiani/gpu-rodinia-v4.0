...
__global__ void calculate_temp(int iteration,  //number of iteration
        ...
        if(IN_RANGE(loadYidx, 0, grid_rows-1) && IN_RANGE(loadXidx, 0, grid_cols-1)){
            ...
            local_power = power[index];// Load the power data from global memory to shared memory
        }
        __syncthreads();
        ...
        float NS = step_div_Cap * Ry_1;
        float EW = step_div_Cap * Rx_1;
        float Amb = step_div_Cap * Rz_1 * amb_temp;
        float PowerAndAmb = local_power * step_div_Cap + Amb;
        float Center = 1.0f - step_div_Cap * (2.0f * (Ry_1 + Rx_1) + Rz_1);
        ...
        for (int i=0; i<iteration ; i++){
            ...
            if( IN_RANGE(tx, i+1, BLOCK_SIZE-i-2) &&  \
                  ...
                  temp_t[ty][tx] = temp_on_cuda[ty][tx] * Center
                     + (temp_on_cuda[S][tx] + temp_on_cuda[N][tx]) * NS
                     + (temp_on_cuda[ty][E] + temp_on_cuda[ty][W]) * EW
                     + PowerAndAmb;
            }
            __syncthreads();
            if(i==iteration-1)
