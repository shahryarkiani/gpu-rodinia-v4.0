// Compile: nvcc -O3 -std=c++17 -arch=sm_80 bfs_gunrock_style_opt_fixed.cu -o bfs_gunrock_style_opt_fixed -lnvToolsExt
// Run:     ./bfs_gunrock_style_opt_fixed <rodinia_csr_graph.txt>
...
int main(int argc, char** argv) {
    ...
    // Host Memory Allocation & Initialization Phase (Pinned Memory)
    eventAttrib.color = NVTX_COLOR_YELLOW;
    eventAttrib.message.ascii = "Host Memory Setup (Pinned)";
    nvtxRangePushEx(&eventAttrib);

    // Allocate pinned memory for distance array and bitset
    int* h_dist = nullptr;
    ...
    // Host to Device Memory Transfer Phase (Async with Streams)
    eventAttrib.color = NVTX_COLOR_CYAN;
    eventAttrib.message.ascii = "Async Host to Device Memory Transfer";
    nvtxRangePushEx(&eventAttrib);

    // Create CUDA streams for overlapping transfers
    cudaStream_t stream1, stream2, stream3;
    CUDA_CHECK(cudaStreamCreate(&stream1));
    CUDA_CHECK(cudaStreamCreate(&stream2));
    CUDA_CHECK(cudaStreamCreate(&stream3));

    // Stream 1: OUT-graph (row offsets + column indices) - data already in pinned memory
    ...
    // Stream 2: IN-graph (transpose - row offsets + column indices) - data already in pinned memory
    ...
    // Stream 3: Distance array and initialization memsets
    ...
    eventAttrib.message.ascii = "BFS Initialization";
    nvtxRangePushEx(&eventAttrib);
    ...
    while (true) {
        ...
        sprintf(iteration_msg, "BFS Iteration %d", iters);
        ...
        if (in_pull) {
            nvtxEventAttributes_t pullAttrib = {0};
            pullAttrib.version = NVTX_VERSION;
            pullAttrib.size = NVTX_EVENT_ATTRIB_STRUCT_SIZE;
            pullAttrib.colorType = NVTX_COLOR_ARGB;
            pullAttrib.color = NVTX_COLOR_BLUE;
            pullAttrib.messageType = NVTX_MESSAGE_TYPE_ASCII;
            pullAttrib.message.ascii = "PULL Phase";
            nvtxRangePushEx(&pullAttrib);
            // Epoch-mark current frontier (no memset)
            ...
            nvtxRangePop(); // End PULL Phase
        } else {
            nvtxEventAttributes_t pushAttrib = {0};
            pushAttrib.version = NVTX_VERSION;
            pushAttrib.size = NVTX_EVENT_ATTRIB_STRUCT_SIZE;
            pushAttrib.colorType = NVTX_COLOR_ARGB;
            pushAttrib.color = NVTX_COLOR_GREEN;
            pushAttrib.messageType = NVTX_MESSAGE_TYPE_ASCII;
            pushAttrib.message.ascii = "PUSH Phase";
            nvtxRangePushEx(&pushAttrib);
            // PUSH: bucket by degree
            ...
            // Small
            if (h_smallN > 0) {
                nvtxEventAttributes_t smallAttrib = {0};
                smallAttrib.version = NVTX_VERSION;
                smallAttrib.size = NVTX_EVENT_ATTRIB_STRUCT_SIZE;
                smallAttrib.colorType = NVTX_COLOR_ARGB;
                smallAttrib.color = NVTX_COLOR_CYAN;
                smallAttrib.messageType = NVTX_MESSAGE_TYPE_ASCII;
                smallAttrib.message.ascii = "Push Small Kernel";
                nvtxRangePushEx(&smallAttrib);
            ...
            // Medium: launch an occupancy-friendly grid; kernel grid-strides over WARPS
            if (h_medN > 0) {
                nvtxEventAttributes_t medAttrib = {0};
                medAttrib.version = NVTX_VERSION;
                medAttrib.size = NVTX_EVENT_ATTRIB_STRUCT_SIZE;
                medAttrib.colorType = NVTX_COLOR_ARGB;
                medAttrib.color = NVTX_COLOR_YELLOW;
                medAttrib.messageType = NVTX_MESSAGE_TYPE_ASCII;
                medAttrib.message.ascii = "Push Medium Kernel";
                nvtxRangePushEx(&medAttrib);
                int threads = TPB;                 // multiple warps per block
            ...
            // Large: launch an occupancy-friendly grid; kernel grid-strides over CTAs
            if (h_largeN > 0) {
                nvtxEventAttributes_t largeAttrib = {0};
                largeAttrib.version = NVTX_VERSION;
                largeAttrib.size = NVTX_EVENT_ATTRIB_STRUCT_SIZE;
                largeAttrib.colorType = NVTX_COLOR_ARGB;
                largeAttrib.color = NVTX_COLOR_PURPLE;
                largeAttrib.messageType = NVTX_MESSAGE_TYPE_ASCII;
                largeAttrib.message.ascii = "Push Large Kernel";
                nvtxRangePushEx(&largeAttrib);
                int blocks = std::min(MAX_BLOCKS, std::max(1, prop.multiProcessorCount*6));
        ...
        if (iters > G.n) { fprintf(stderr,"[warn] iterations > |V|; break\n"); break; }
    }
    ...
    write_result("result_gunrock_style.txt", h_dist, G.n);
    ...
    cudaFreeHost(h_dist);
    // G's memory (row_off, col_idx, row_off_T, col_idx_T) freed by destructor

    nvtxRangePop(); // End Memory Cleanup

    std::printf("BFS done. Levels=%d, iters=%d (%s)\n", level, iters, "direction-optimized");

    nvtxRangePop(); // End BFS Main Program
    return 0;
}